var dialog = [];
dialog.push("");
dialog.push("Only junk mail, per usual.");
...
var fountain=({name:"fountain", x:970, y:25, width:85, height:115, imgOffset:0});
var libraryDoor=({name:"libraryDoor", x:1115, y:10, width:70, height:180, open:true});
...
var tree=({name:"tree", x:-589, y:-365, width:175, height:350, frames:2, imgOffset:0});
...
movingObjects.push(globe,busts,library,ladder,tree,ball,mailbox,dancer,archer,fountain,libraryDoor,pinkGirl,building,house,yellowGirl,skyTop,skyBottom);
...
pics.push(library,ladder,tree,dancer,archer,fountain,libraryDoor,pinkGirl,yellowGirl,player,globe,busts,ball,mailbox,building,house);
...
for (i = 0; i < rungs.length; i++){
  ...
  //fountain cycle
  if(frameCount%5===0){
  	if(fountain.imgffset < fountain.width*2){
...
function anim(pic, frames, speed, direction){
  //draw non-interactive animated images
